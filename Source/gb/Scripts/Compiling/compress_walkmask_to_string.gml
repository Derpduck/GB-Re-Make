{
    ...
    width = background_get_width(LevelWMB)
    height = background_get_height(LevelWMB)

    //Write the walkmask background to a surface.
    wm_surface = surface_create(background_get_width(LevelWMB), background_get_height(LevelWMB))

    //Draw the walkmask onto the surface.
    surface_set_target(wm_surface)
    draw_set_color(c_aqua)
    draw_rectangle(0,0,width,height,false)
    draw_background(LevelWMB,0,0)
    ...
    wm_sprite = sprite_create_from_surface(wm_surface,0,0,background_get_width(LevelWMB),background_get_height(LevelWMB),true,false,0,0)
    ...
    for(a = 0; a < height; a += 1){
        draw_progress_bar(a / height * 100, "Compressing Walkmask")
        screen_refresh()
        for(b = 0; b < width; b += 1){
